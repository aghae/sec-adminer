...
class AdminerTinymce {
	...
	* @param string
	*/
	function __construct($path = "../tinymce/tinymce.min.js") {
		$this->path = $path;
	}
...
<?php
	...
	function selectVal(&$val, $link, $field, $original) {
		...
		if (preg_match("~_html~", $field["field"]) && $val != '') {
			...
			if (class_exists('DOMDocument')) { // close all opened tags
				$dom = new DOMDocument;
				if (@$dom->loadHTML("<meta http-equiv='Content-Type' content='text/html; charset=utf-8'><meta http-equiv='Content-Security-Policy' content='default-src *;'></head>$val")) { // @ - $val can contain errors
	...
	function editInput($table, $field, $attrs, $value) {
		if (preg_match("~text~", $field["type"]) && preg_match("~_html~", $field["field"])) {
			return "<textarea$attrs id='fields-" . h($field["field"]) . "' rows='12' cols='50'  class='mceEditor'>" . h($value) . "</textarea>" . script("
